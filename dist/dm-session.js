...
function buildDMSystemPrompt(rules, state, npcs) {
    const npcIntro = Object.entries(npcs).map(([id, def]) => {
        const parts = [`${def.display}（${id}）`];
        if (def.relationship)
            parts.push(`与睦的关系：${def.relationship}`);
        if (def.description)
...
export async function recoverFromCrash(dataDir, locations, network, npcs) {
    ...
    if (state._mutsumi.position.type === "traveling") {
        ...
        const { arrived } = advanceTraveling(state._mutsumi.position, gapMs, 1.2, totalDist);
        if (arrived) {
