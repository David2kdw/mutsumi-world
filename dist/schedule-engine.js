 ...
 */
export function expandSchedule(template, dateStr) {
 ...
 * 找到当前时间对应的日程段。
 */
export function findCurrentSegment(schedule, time) {
 ...
 * 找到当前段之后的下一个日程段。
 */
export function findNextSegment(schedule, time) {
    const tMin = timeToMinutes(time);
    for (const entry of schedule) {
        if (timeToMinutes(entry.start) > tMin)
            return entry;
    }
    return null;
}
