 ...
 */
function parseDailyChatLog(sessionsDir, dateStr) {
    const allLines = [];
    try {
...
export async function generateDiary(dataDir, workspaceDir, llmClient, soulPath) {
    ...
    if (trajectory.length === 0)
        return;
    const sessionsDir = path.join(path.dirname(workspaceDir), "agents", "main", "sessions");
