...
下一段：08:00 教室 | 数学（距离约125m，步行约2分钟）`;
    ...
    assert.ok(["none", "stay", "event", "move"].includes(response.action),
      `action should be valid, got: ${response.action}`);
    assert.ok(typeof response.environment === "string" && response.environment.length > 0,
